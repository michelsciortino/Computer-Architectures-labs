...
Reset_Handler   PROC
				...
				; your code here
Price_list	DCD 0x042, 230, 0x04A, 265, 0x007, 1210, 0x00A, 245 
			...
			DCD 0x004, 120, 0x045, 1112, 0x047, 2627 , 0x006, 315
	
Item_list 	DCD 0x022, 14, 0x006, 431, 0x03E, 1210, 0x017, 56342 

		MOV R0,	#8			;item struct size
		MOV R1,	#4			;number of elements in the list
		MOV R2,	#0			;current element
		MOV R10,#0			;tot=0
		MUL R9,R0,R1		;total lenght
		LDR R11,=Price_list
		LDR R12,=Item_list
CopyMatrix
			LDR R4,=Heap_Mem
			MOV R3,#0	 	;index
			MOV R5,#28 	 	;remaining items 
CopyLoop			  
			LDR R6,[R11,R3]
			STR R6,[R4,R3]
			ADD R3,#4
...
Bynary_search	
		...
		MOV R5,R5, LSR #1
		MUL R8,R5,R0 			;index
		LDR R7,[R11,R8]	  		;reading from matrix
		cmp R6,R7				;key check
		BEQ Use_Result
		ADDLT R4,R5,#-1
		ADDGT R3,R5,#+1
...
Use_Result	
		ADD R8, #4
		ADD R2, #4
		LDR R6, [R12,R2]		;get item quantity
		LDR R7, [R11,R8]		;get item price
		MUL R8, R6, R7
		ADD R10, R8
		ADD R2, #4
		CMP R2,R9				;loop closing checking
		BEQ End_Program
		B Iteration
...
InfLoop         B      	InfLoop
		...
		;receive start index in R11 and lenght in R10
Sort	PUSH {r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,LR}
		MOV R0,#0	;i
		ADD R10,R10,#-1
		LSL R10,#3
		ADD R2,R10,#-8
...
internalLoop
		...
		LDR R4,[R11,R1]		;arr[j]
		ADD R3,R1,#8	  	;j+1
		LDR R5,[R11,R3]		;arr[j+1]
		CMP R4,R5		  	;if (arr[j] > arr[j+1]) 
		STRGT R4,[R11,R3] 	;swap(&arr[j], &arr[j+1]);
		STRGT R5,[R11,R1]	;		
		ADDGT R3,R1,#4		;
		ADDGT R6,R3,#8		;
		LDRGT R4,[R11,R3]	;
